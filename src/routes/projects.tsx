...
function ProjectsPage() {
  ...
  const allProjects = storeProjects !== null ? storeProjects : PROJECTS_DATA;
  const storeCategories = store.getProjectCategories();
  const categories = ["All", ...(storeCategories && storeCategories.length > 0 ? storeCategories : ["Government & Municipal", "Manufacturing & Heavy Industry", "Hospitals & Healthcare", "Banks & Financial"])];

  const [activeCategory, setActiveCategory] = useState("All");
  const [isPaused, setIsPaused] = useState(false);
  const projectsSliderRef = useRef<HTMLDivElement>(null);

  // Automatic auto-play slider effect for 'All' category
  useEffect(() => {
  ...
  const filteredProjects =
    activeCategory === "All"
      ? allProjects
      : allProjects.filter((p) => p.categoryTag === activeCategory);
  ...
  return (
    <div className="bg-[#F8FAFC] text-slate-900 font-sans antialiased min-h-screen pt-4 pb-12 relative overflow-hidden">
      ...
      <div className="max-w-[1536px] mx-auto px-4 sm:px-6 lg:px-8 space-y-6 relative z-10">
        {/* HERO BANNER CARD */}
        <div className="bg-white border border-slate-200/80 rounded-[28px] p-4 sm:p-6 lg:p-7 shadow-sm relative overflow-hidden space-y-4 sm:space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-center relative z-10">
            <div className="lg:col-span-6 space-y-3 sm:space-y-3.5">
              <div className="inline-flex items-center gap-2 px-3 py-0.5 rounded-full bg-red-50 text-red-600 text-[10px] sm:text-[10.5px] font-black uppercase tracking-widest border border-red-100">
                <Building2 className="size-3.5 text-red-600" />
                <span>LANDMARK DEPLOYMENTS</span>
              </div>

              <div className="space-y-1">
                <h1 className="text-2xl sm:text-3xl lg:text-4xl font-black text-slate-900 tracking-tight leading-[1.15] sm:leading-[1.12]">
                  Proven Infrastructure <br className="hidden sm:inline" />
              ...
              <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 pt-0.5 text-[10px] sm:text-[11px] font-bold text-slate-900">
            ...
            <div className="lg:col-span-6 flex items-center justify-end relative">
              <div className="relative flex items-center gap-2 sm:gap-4 w-full max-w-xl justify-end pr-0 sm:pr-24 pt-1 sm:pt-0">
                <div className="h-44 sm:h-60 lg:h-[255px] w-1/3 overflow-hidden rounded-[18px] sm:rounded-[20px] transform -skew-x-12 border-2 border-white shadow-lg group bg-slate-950 relative">
                  <img
                  ...
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-slate-950/95 via-slate-950/40 to-transparent flex flex-col justify-end p-2.5 sm:p-3.5 transform skew-x-12">
                    <div className="w-4 sm:w-5 h-0.5 bg-red-500 mb-1 sm:mb-1.5 rounded-full" />
                    <div className="text-[11px] sm:text-xs font-black text-white leading-tight">Smart Cities</div>
                  </div>
                </div>
                ...
                <div className="h-44 sm:h-60 lg:h-[255px] w-1/3 overflow-hidden rounded-[18px] sm:rounded-[20px] transform -skew-x-12 border-2 border-white shadow-lg group bg-slate-950 relative">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-slate-950/95 via-slate-950/40 to-transparent flex flex-col justify-end p-2.5 sm:p-3.5 transform skew-x-12">
                    <div className="w-4 sm:w-5 h-0.5 bg-red-500 mb-1 sm:mb-1.5 rounded-full" />
                    <div className="text-[11px] sm:text-xs font-black text-white leading-tight">Industrial</div>
                  </div>
                </div>
                ...
                <div className="h-44 sm:h-60 lg:h-[255px] w-1/3 overflow-hidden rounded-[18px] sm:rounded-[20px] transform -skew-x-12 border-2 border-white shadow-lg group bg-slate-950 relative">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-slate-950/95 via-slate-950/40 to-transparent flex flex-col justify-end p-2.5 sm:p-3.5 transform skew-x-12">
                    <div className="w-4 sm:w-5 h-0.5 bg-red-500 mb-1 sm:mb-1.5 rounded-full" />
                    <div className="text-[11px] sm:text-xs font-black text-white leading-tight">Healthcare</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="bg-slate-50 border border-slate-200/80 rounded-[20px] p-3.5 sm:px-6 sm:py-3 flex flex-col md:flex-row md:items-center justify-between gap-3 sm:gap-4 relative z-10">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-6 items-center">
              <div className="flex items-center gap-2.5">
                <div className="size-8 sm:size-9 rounded-full bg-red-50 flex items-center justify-center text-red-600 shrink-0 border border-red-100">
                  <Video className="size-3.5 sm:size-4" />
                </div>
                <div>
                  <div className="text-sm sm:text-base font-black text-slate-900">240+</div>
                  <div className="text-[8.5px] sm:text-[9px] font-black uppercase tracking-wider text-slate-400">4K CAMERAS</div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <div className="text-sm sm:text-base font-black text-slate-900">15 km</div>
                  <div className="text-[8.5px] sm:text-[9px] font-black uppercase tracking-wider text-slate-400">FIBER RING</div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <div className="text-sm sm:text-base font-black text-slate-900">32</div>
                  <div className="text-[8.5px] sm:text-[9px] font-black uppercase tracking-wider text-slate-400">BANK BRANCHES</div>
                </div>
              </div>

              <div className="flex items-center gap-2.5">
                <div className="size-8 sm:size-9 rounded-full bg-red-50 flex items-center justify-center text-red-600 shrink-0 border border-red-100">
                  <ShieldCheck className="size-3.5 sm:size-4 text-red-600" />
                </div>
                <div>
                  <div className="text-sm sm:text-base font-black text-red-600">99.98%</div>
                  <div className="text-[8.5px] sm:text-[9px] font-black uppercase tracking-wider text-slate-400">UPTIME SLA</div>
                </div>
              </div>
            </div>

            <div className="flex items-center justify-between sm:justify-start gap-3 bg-white border border-slate-200 px-3.5 sm:px-4 py-2 rounded-full w-full sm:w-auto">
              <span className="text-[9.5px] sm:text-[10px] font-black uppercase tracking-wider text-slate-500">
                INFRASTRUCTURE FOR MAHARASHTRA
              </span>
              <div className="size-6 sm:size-7 rounded-full bg-red-600 flex items-center justify-center text-white shrink-0">
        ...
        {/* FILTER TABS */}
        <div className="flex items-center justify-between gap-3 sm:gap-4 flex-wrap bg-white border border-slate-200/80 rounded-2xl p-2.5 sm:p-3 shadow-sm">
            ...
            {categories.map((cat) => {
              ...
              return (
                <button
                  ...
                  className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-full text-xs font-extrabold transition-all duration-300 cursor-pointer ${isActive
          ...
          <div className="text-[11px] sm:text-xs text-slate-400 font-bold px-2">
            Showing {filteredProjects.length} Projects
          </div>
        </div>

        {/* PROJECTS CONTAINER - SLIDER (1 CARD AT A TIME) ON MOBILE FOR 'ALL', GRID FOR SPECIFIC CATEGORIES */}
        <div
          ...
          className={
            activeCategory === "All"
              ? "flex overflow-x-auto snap-x snap-mandatory gap-4 pb-4 select-none scrollbar-none scroll-smooth md:grid md:grid-cols-2 md:gap-6 md:pb-6 md:pt-1"
              : "grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 pb-6 pt-1"
          }
        >
          {filteredProjects.map((project) => (
            <div
              ...
              className={
                activeCategory === "All"
                  ? "flex-none w-full snap-center bg-white border border-slate-200/80 rounded-[23px] flex flex-col justify-between overflow-hidden group shadow-sm hover:shadow-md transition-shadow"
                  : "w-full bg-white border border-slate-200/80 rounded-[23px] flex flex-col justify-between overflow-hidden group shadow-sm hover:shadow-md transition-shadow"
              }
            >
              <div>
                <div className="relative h-52 sm:h-64 w-full bg-slate-950 overflow-hidden">
                  ...
                  />
                  <div className="absolute top-3 left-3 sm:top-4 sm:left-4 bg-red-600 text-white text-[9px] sm:text-[10px] font-black uppercase tracking-widest px-2.5 sm:px-3.5 py-1 sm:py-1.5 rounded-full shadow-md flex items-center gap-1 sm:gap-1.5">
                    <Sparkles className="size-3 text-white" />
                    <span>{project.categoryBadge}</span>
                  </div>

                  <div className="absolute bottom-3 left-3 sm:bottom-4 sm:left-4 bg-slate-950/90 text-white text-[11px] sm:text-xs font-bold px-3 sm:px-3.5 py-1 sm:py-1.5 rounded-full flex items-center gap-1.5 sm:gap-2 border border-slate-800">
                    <MapPin className="size-3 sm:size-3.5 text-red-400" />
                    <span>{project.location}</span>
                  </div>
                </div>

                <div className="p-4 sm:p-6 space-y-3 sm:space-y-4">
                  <div className="text-[10px] sm:text-[11px] font-black uppercase tracking-wider text-red-600">
                    {project.scale}
                  </div>
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-2xl p-2.5 sm:p-3 grid grid-cols-2 sm:grid-cols-4 gap-2 text-center">
                    {project.metrics?.map((m, idx) => (
                      <div
                        key={idx}
                        className="bg-white rounded-xl p-1.5 sm:p-2 border border-slate-200"
                      >
                        <div className="text-xs font-black text-slate-900">
                          {m.value}
                        </div>
                        <div className="text-[8px] sm:text-[8.5px] font-extrabold uppercase tracking-wider text-slate-400 mt-0.5">
                          {m.label}
                        </div>
                      </div>
                    ))}
                  ...
                  {project.detailedSpecs && project.detailedSpecs.length > 0 && (
                    <div className="pt-2 space-y-2">
                      <div className="text-[10.5px] sm:text-[11px] font-black text-slate-900 uppercase tracking-wider flex items-center gap-1.5">
                        <Cpu className="size-3.5 text-red-600" />
                        <span>Technical Specifications &amp; Architecture</span>
                      </div>

                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        {project.detailedSpecs.map((spec, idx) => (
                          <div
                            key={idx}
                            className="bg-slate-50 border border-slate-200 rounded-xl p-2.5 flex items-start gap-2 text-[11px] text-slate-700 font-medium"
                          >
              ...
              <div className="p-4 sm:p-6 pt-0 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-t border-slate-200 mt-2">
                <div className="flex items-center gap-1.5 sm:gap-2 text-xs font-extrabold text-red-600">
                ...
                <Link
                  to="/contact"
                  className="w-full sm:w-auto px-4 py-2.5 rounded-xl bg-red-600 hover:bg-red-700 text-white text-xs font-extrabold flex items-center justify-center gap-1.5 shadow-md shadow-red-600/20 transition cursor-pointer text-center"
                >
