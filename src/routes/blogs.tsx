      ...
      <div className="max-w-7xl mx-auto px-3.5 sm:px-6 lg:px-8 space-y-6 sm:space-y-8">
        ...
        <div className="space-y-2.5 sm:space-y-3">
          ...
          <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider">
            ...
            <span className="text-sky-600 font-bold">TECHNICAL BLOGS & NORMS</span>
          </div>
          ...
          <p className="text-slate-600 text-xs sm:text-base max-w-4xl font-normal leading-relaxed">
            Practical guidance written by JEPL senior engineers on meeting statutory RBI physical security guidelines, NBC 2016 fire standards, and optical fiber network topology design.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none pt-1">
          {tags.map((tag) => (
            <button
              ...
              className={`px-3.5 py-2 rounded-full text-xs font-extrabold transition cursor-pointer shrink-0 ${
                selectedTag === tag
                  ? "bg-[#06143D] text-white shadow-sm"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6 pt-1">
          {filteredPosts.map((post) => (
            <div
              key={post.id}
              className="bg-white border border-slate-200/90 rounded-3xl p-5 sm:p-7 shadow-xs hover:shadow-md transition duration-300 flex flex-col justify-between space-y-5 sm:space-y-6"
            >
              <div className="space-y-4">
                {/* Header Tag & Read Time */}
                <div className="flex items-center justify-between gap-2">
                  <span className="bg-sky-50 text-sky-700 border border-sky-100 px-3 py-1 rounded-md text-[10px] font-black uppercase tracking-wider">
                    {post.tag}
                  </span>
                ...
                {/* Article Title */}
                <h2 className="text-base sm:text-xl font-black text-slate-900 leading-snug tracking-tight hover:text-sky-600 transition cursor-pointer">
                  {post.title}
                </h2>
              ...
              <div className="flex items-center justify-between pt-4 border-t border-slate-100 text-xs font-semibold">
                ...
                <Link
                  to="/contact"
                  className="text-sky-600 font-extrabold flex items-center gap-1 hover:gap-2 transition-all group"
                >
