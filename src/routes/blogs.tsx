...
const DEFAULT_BLOG_POSTS: BlogPost[] = [
  ...
  {
    id: "biometric-access-anti-passback",
    tag: "ACCESS CONTROL & BIOMETRICS",
    readTime: "5 min read",
    title: "Biometric Access Control & Time Attendance: Best Practices for High-Security Facilities",
    description:
      "How dual-factor biometric access control, facial recognition, and anti-passback rules protect sensitive data centers and corporate headquarters in Maharashtra.",
    takeaway:
      "Anti-passback prevents credential sharing and ensures 100% audit accuracy for statutory compliance.",
    date: "May 30, 2026",
    image:
      "https://images.unsplash.com/photo-1558002038-1055907df827?auto=format&fit=crop&w=600&q=80",
    content:
      "Enterprise security architectures require robust multi-factor authentication combining RFID smartcards with AI facial recognition nodes. Implementing hard anti-passback logic ensures physical entry logs remain airtight against unauthorized tailgating.",
  },
];
...
function BlogsPage() {
  ...
  const blogsSliderRef = useRef<HTMLDivElement>(null);

  // Automatic auto-play slider effect for 'All' category
  useEffect(() => {
    if (selectedTag !== "All" || isPaused) return;

    const timer = setInterval(() => {
      if (blogsSliderRef.current) {
        const container = blogsSliderRef.current;
  ...
  const localStoreBlogs = store.getBlogs();
  ...
  const combinedPosts: BlogPost[] = firestoreBlogs.length > 0
    ...
    : localStoreBlogs && localStoreBlogs.length > 0
    ? localStoreBlogs
    : DEFAULT_BLOG_POSTS;

  const storeBlogCategories = store.getBlogCategories();
  const categories = [
    "All",
    ...(storeBlogCategories && storeBlogCategories.length > 0
      ? storeBlogCategories
      : ["Compliance & Banking", "Networking & Fiber", "Smart City & AI"])
  ];
    ...
    <div className="bg-[#FFFFFF] text-slate-800 font-sans antialiased min-h-screen pt-4 pb-16 relative overflow-hidden">
      ...
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 space-y-6 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 items-stretch pt-2">
          ...
          <div className="lg:col-span-4 bg-gradient-to-br from-white via-slate-50/50 to-red-50/30 border border-slate-200/90 rounded-3xl p-5 sm:p-8 flex flex-col justify-between space-y-5 sm:space-y-6 shadow-xs relative overflow-hidden h-full">
            <div className="space-y-4 sm:space-y-5 relative z-10">
              <div className="inline-flex items-center gap-1.5 text-[10px] sm:text-xs font-black uppercase tracking-wider text-[#DC2626] bg-red-50 px-3 sm:px-3.5 py-1 rounded-full border border-red-100 shadow-2xs">
                <span>TECHNICAL BLOGS &amp; NORMS</span>
              </div>

              <h1 className="text-2xl sm:text-4xl font-black text-slate-900 tracking-tight leading-[1.15]">
              ...
              <div className="space-y-2.5 sm:space-y-3 pt-1 sm:pt-2">
                <div className="bg-white border border-slate-200/80 rounded-2xl p-2.5 sm:p-3 flex items-center gap-3 sm:gap-3.5 shadow-2xs hover:shadow-md transition">
                ...
                <div className="bg-white border border-slate-200/80 rounded-2xl p-2.5 sm:p-3 flex items-center gap-3 sm:gap-3.5 shadow-2xs hover:shadow-md transition">
                  <div className="p-2 sm:p-2.5 rounded-xl bg-red-50 text-[#DC2626] shrink-0 border border-red-100">
                    <FileText className="size-4 sm:size-4.5 text-[#DC2626]" />
                  </div>
                  <span className="text-[11px] sm:text-xs font-extrabold text-slate-900">
                    RBI &amp; NBC Guidelines
                  </span>
                ...
                <div className="bg-white border border-slate-200/80 rounded-2xl p-2.5 sm:p-3 flex items-center gap-3 sm:gap-3.5 shadow-2xs hover:shadow-md transition">
                  <div className="p-2 sm:p-2.5 rounded-xl bg-red-50 text-[#DC2626] shrink-0 border border-red-100">
                    <CheckCircle2 className="size-4 sm:size-4.5 text-[#DC2626]" />
                  </div>
                  <span className="text-[11px] sm:text-xs font-extrabold text-slate-900">
                ...
                <div className="bg-white border border-slate-200/80 rounded-2xl p-2.5 sm:p-3 flex items-center gap-3 sm:gap-3.5 shadow-2xs hover:shadow-md transition">
                  <div className="p-2 sm:p-2.5 rounded-xl bg-red-50 text-[#DC2626] shrink-0 border border-red-100">
                    <Award className="size-4 sm:size-4.5 text-[#DC2626]" />
                  </div>
                  <span className="text-[11px] sm:text-xs font-extrabold text-slate-900">
            ...
            <div className="pt-3 sm:pt-4 border-t border-slate-200/80 relative z-10 flex flex-col items-start space-y-1">
              <span className="text-sm sm:text-lg font-serif italic text-slate-900 font-bold tracking-wide">
          ...
          <div className="lg:col-span-8 space-y-4 sm:space-y-5 flex flex-col justify-between">
            <div className="space-y-4 sm:space-y-5">
              ...
              <div className="relative flex items-center">
                <Search className="absolute left-4 size-4 sm:size-4.5 text-slate-400 pointer-events-none" />
                <input
                  type="text"
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  placeholder="Search articles, topics..."
                  className="w-full bg-white border border-slate-200/90 rounded-full py-3 sm:py-3.5 pl-11 pr-13 text-xs sm:text-sm text-slate-900 placeholder:text-slate-400 shadow-2xs focus:outline-none focus:border-[#DC2626] focus:ring-4 focus:ring-red-600/10 transition font-medium"
                />
                <button
                  type="button"
                  className="absolute right-1.5 size-8.5 sm:size-9.5 rounded-full bg-[#DC2626] hover:bg-[#B91C1C] text-white flex items-center justify-center shadow-md transition cursor-pointer"
                  title="Search"
                >
                ...
                {categories.map((cat) => {
                  ...
                  return (
                    <button
                      ...
                      className={`px-3.5 sm:px-4 py-1.5 sm:py-2 rounded-full text-xs font-extrabold transition cursor-pointer ${
                        isActive
              ...
              <div
                ...
                className={
                  selectedTag === "All"
                    ? "flex lg:block overflow-x-auto lg:overflow-visible pb-4 lg:pb-0 gap-4 lg:gap-0 lg:space-y-4 snap-x snap-mandatory scroll-smooth select-none scrollbar-none"
                    : "block space-y-4"
                }
              >
                {filteredPosts.length === 0 ? (
                  ...
                  filteredPosts.map((post) => (
                    <div
                      ...
                      className={
                        selectedTag === "All"
                          ? "w-full lg:w-auto shrink-0 snap-center bg-white border border-slate-200/80 hover:border-red-400 rounded-3xl p-4 sm:p-5 shadow-2xs hover:shadow-md transition-all duration-300 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-5 group cursor-pointer"
                          : "w-full bg-white border border-slate-200/80 hover:border-red-400 rounded-3xl p-4 sm:p-5 shadow-2xs hover:shadow-md transition-all duration-300 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-5 group cursor-pointer"
                      }
                    >
                      <div className="w-full sm:w-44 h-40 sm:h-32 rounded-2xl overflow-hidden bg-slate-950 shrink-0 relative">
                        <img
                          src={post.image}
                          alt={post.title}
                      ...
                      <div className="flex-1 space-y-1.5 sm:space-y-2">
                        <div className="flex items-center gap-2.5 sm:gap-3 flex-wrap">
                          <span className="bg-red-50 text-[#DC2626] border border-red-100/80 px-2.5 sm:px-3 py-0.5 rounded-full text-[9.5px] sm:text-[10px] font-black tracking-wider uppercase">
                            {post.tag}
                          </span>
                        </div>

                        <h2 className="text-sm sm:text-lg font-black text-slate-900 group-hover:text-[#DC2626] transition-colors leading-snug">
                          {post.title}
                        </h2>
                      ...
                      <div className="shrink-0 self-end sm:self-center">
                        <div className="size-9 sm:size-10 rounded-full bg-red-50 text-[#DC2626] group-hover:bg-[#DC2626] group-hover:text-white flex items-center justify-center transition-all duration-300 shadow-2xs">
      ...
      {activeArticle && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3.5 sm:p-4 bg-slate-950/60 backdrop-blur-xs">
          <div className="bg-white rounded-3xl max-w-2xl w-full p-4 sm:p-8 space-y-4 sm:space-y-6 max-h-[90vh] overflow-y-auto border border-slate-200/90 relative shadow-2xl">
            <button
              onClick={() => setActiveArticle(null)}
              className="absolute top-4 right-4 sm:top-5 sm:right-5 p-2 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-800 transition cursor-pointer"
            >
              <X className="size-4 sm:size-5" />
            </button>

            <div className="space-y-2 sm:space-y-3 pr-8">
              <span className="bg-red-50 text-[#DC2626] border border-red-100 px-3 py-0.5 sm:px-3.5 sm:py-1 rounded-full text-[9px] sm:text-[10px] font-black uppercase tracking-wider">
                {activeArticle.tag}
              </span>
            ...
            <div className="h-48 sm:h-60 w-full rounded-2xl overflow-hidden bg-slate-950">
              <img
            ...
            <div className="space-y-3 sm:space-y-4 text-xs sm:text-sm text-slate-700 leading-relaxed font-normal">
              <p>{activeArticle.description}</p>
              {activeArticle.content && <p className="pt-1 sm:pt-2">{activeArticle.content}</p>}

              <div className="bg-red-50/70 border border-red-100 rounded-2xl p-3.5 sm:p-4 space-y-1 sm:space-y-1.5">
                <div className="text-[10px] sm:text-[11px] font-black uppercase text-[#DC2626] flex items-center gap-1.5">
                  <Sparkles className="size-3 sm:size-3.5" />
                  <span>KEY ENGINEERING TAKEAWAY</span>
                </div>
                <p className="text-[11px] sm:text-xs text-slate-900 font-bold leading-relaxed">
            ...
            <div className="pt-4 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-3">
              <Link
                to="/contact"
                onClick={() => setActiveArticle(null)}
                className="w-full sm:w-auto px-6 py-3 rounded-xl bg-[#DC2626] hover:bg-[#B91C1C] text-white text-xs font-extrabold flex items-center justify-center gap-2 shadow-md transition"
              >
              ...
              <button
                onClick={() => setActiveArticle(null)}
                className="w-full sm:w-auto px-4 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-bold transition cursor-pointer"
              >
                Close Article
              </button>
            </div>
